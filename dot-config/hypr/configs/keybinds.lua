...
hl.bind("SUPER + P", hl.dsp.window.pseudo()) -- dwindle layout only
...
for i = 1, 4 do
	...
	hl.bind("ALT + F" .. button, hl.dsp.window.move({ workspace = i, follow = true }))
	hl.bind("SUPER + CTRL + F" .. button, hl.dsp.window.move({ workspace = i, follow = false }))
end
...
hl.bind(
	"CTRL + SHIFT + 1 ",
	hl.dsp.exec_cmd(
		"pic=$(xdg-user-dir PICTURES)/grim/$(date '+%Y-%m-%dT%H-%M-%S.png'); grim - > $pic ; wl-copy -t image/png < $pic"
	)
)
hl.bind(
	"CTRL + SHIFT + 2 ",
	hl.dsp.exec_cmd(
		"pic=$(xdg-user-dir PICTURES)/grim/$(date '+%Y-%m-%dT%H-%M-%S.png'); grim -o $(hyprctl activeworkspace | rg \"DP-\\d\" -o) - > $pic; wl-copy -t image/png < $pic"
	)
)
hl.bind(
	"CTRL + SHIFT + 4 ",
	hl.dsp.exec_cmd(
		"pic=$(xdg-user-dir PICTURES)/grim/$(date '+%Y-%m-%dT%H-%M-%S.png'); grim -g \"$(slurp)\" - > $pic; wl-copy -t image/png < $pic"
	)
)
...
-- volume #
--f3, f4
hl.bind("xf86audiomute", hl.dsp.exec_cmd("~/.local/bin/scripts/volume.sh --toggle"))
hl.bind("xf86audiolowervolume", hl.dsp.exec_cmd("~/.local/bin/scripts/volume.sh --dec"))
hl.bind("xf86audioraisevolume", hl.dsp.exec_cmd("~/.local/bin/scripts/volume.sh --inc"))
